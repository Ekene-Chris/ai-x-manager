"""Unified Twitter service that supports both OAuth and legacy API keys"""
...
class TwitterUnifiedService:
    """Unified service for Twitter API that supports both OAuth and legacy auth"""
    ...
    def __init__(self, db_session: Optional[Session] = None):
        ...
        Args:
            db_session: Database session (required for OAuth)
        """
        ...
        if settings.twitter_client_id and settings.twitter_client_secret:
            try:
                self.oauth_service = TwitterOAuthService()
                if db_session:
                    self.client = self.oauth_service.get_client(db_session)
                    if self.client:
                        self.auth_method = "oauth"
                        logger.info("Twitter client initialized with OAuth")
            except Exception as e:
                logger.warning(f"Failed to initialize OAuth: {str(e)}")

        # Fallback to legacy API keys
        if not self.client and settings.twitter_api_key and settings.twitter_api_secret:
            try:
                ...
                logger.info("Twitter client initialized with legacy API keys")
            except Exception as e:
        ...
        if not self.client:
            raise Exception(
                "Twitter authentication not configured. Please set up OAuth or legacy API keys."
            )
    ...
    def get_auth_info(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with auth method and status
        """
        return {
